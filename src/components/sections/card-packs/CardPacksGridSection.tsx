...
// DÜZELTME: Tüm kart renkleri ve başlıkları tasarıma birebir uygun şekilde eşleştirildi!
const cardPacks = [
  ...
  return (
    <section className="w-full bg-[#DFEBF3] pt-[80px] pb-[120px] md:pt-[120px] overflow-hidden">
      ...
      <div className="max-w-[1440px] mx-auto px-[16px] md:px-[40px] lg:px-[80px]">
        ...
        <div className="flex flex-col items-center text-center mb-[60px] relative z-10">
          <h2 className="text-[40px] md:text-[56px] font-extrabold text-[#3A5B93] tracking-tight flex items-center justify-center gap-[12px] mb-[16px]">
            Discover
            <span className="inline-block bg-[#4168A6] text-white px-[24px] py-[4px] rounded-[16px] -rotate-2 shadow-sm">
              cardpacks
            </span>
          </h2>
          <p className="text-[#3A5B93]/80 font-medium text-[18px] mb-[16px]">
            Each pack unlocks a new way to listen, imagine, and play.
          </p>
          <div className="flex items-center justify-center gap-[8px] text-[#3A5B93]/80 text-[14px]">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" className="text-[#3A5B93]"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02z"/></svg>
        ...
        <div className="relative w-full mt-[40px]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-[24px] lg:gap-[32px] relative z-10 justify-items-center">
            {cardPacks.map((pack) => (
              <div 
                key={pack.id} 
                className="w-[302px] h-[427px] bg-white rounded-[30px] p-[10px] flex flex-col gap-[10px] shadow-sm hover:-translate-y-2 transition-transform duration-300 shrink-0"
              >
                {/* Görsel Kutusu (Sahne) */}
                <div 
                  className="w-full h-[266px] rounded-[20px] overflow-hidden relative shrink-0"
                ...
                >
                  ...
                  <img 
                    src={pack.image} 
                    alt={pack.title} 
                    className="absolute top-[8px] left-[56px] w-[169px] h-[266px] object-contain" 
                  />
                </div>

                {/* ALT KISIM: Metinler ve Oynatıcı Bar */}
                <div className="flex flex-col flex-1 px-[6px] pb-[6px]">
                  <h3 className="text-[#505050] font-bold text-[18px] leading-tight mb-[8px]">
                    {pack.title}
                  </h3>
                  
                  {/* Yaş rozeti */}
                  <span className="bg-[#EBB336] text-white text-[13px] font-extrabold px-[12px] py-[4px] rounded-full inline-block self-start shadow-sm mb-auto">
                    {pack.age}
                  </span>

                  {/* Oynatıcı Barları */}
                  {pack.isPlaying ? (
                    <div className="relative w-full h-[39px] rounded-[20px] bg-[#E5EEF4] flex items-center overflow-hidden cursor-pointer mt-[10px] shrink-0">
                      ...
                      <div className="relative z-10 flex items-center gap-[8px] pl-[6px] w-full">
                        <div className="w-[28px] h-[28px] rounded-full bg-[#4168A6] flex items-center justify-center text-white shrink-0 shadow-sm">
                        ...
                        <span className="text-[#4168A6] font-extrabold text-[15px]">Stop</span>
                      </div>
                    </div>
                  ) : (
                    <div className="relative w-full h-[39px] rounded-[20px] bg-[#E5EEF4] flex items-center overflow-hidden cursor-pointer hover:bg-[#D5E3EE] transition-colors mt-[10px] shrink-0">
                      <div className="relative z-10 flex items-center gap-[8px] pl-[6px] w-full">
                        <div className="w-[28px] h-[28px] rounded-full bg-[#4168A6] flex items-center justify-center text-white shrink-0 shadow-sm">
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02z"/></svg>
                        </div>
                        <span className="text-[#4168A6] font-extrabold text-[15px]">Preview</span>
                      </div>
                    </div>
                  )}
                </div>
